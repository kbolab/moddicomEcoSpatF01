#' Moran 2D spatial analysison DICOM images (extension of moddicomV2 package)
  ...
  F01.pipeline.01_zAxis_xFixed<-function(inputGTV, dStar, erosionMarginX=0, erosionMarginY=0, whatDoYouWantBack=c("wMatrix","moranGrayMean","statsI","extractedI","dataFrameI","features")) {
    ...
    for (k in seq(1,length(inputGTV))){
      ...
      # siamo a x costante (piano y-z) --> scam bio z con x e viceversa
      ...
      if (length(listaVoxelErosi) != 0){
        wMatrix<-calcola.matrice.Wz_xFixed(voxelCube,dStar, erosionMarginX, erosionMarginY, vcNonEroso, vcEroso, listaVoxelErosi, pixelSpacingX, pixelSpacingY)
        if("wMatrix" %in% whatDoYouWantBack) final.result$wMatrix[[patID]]<-wMatrix

        # ora calcola il moranGrayMean
        mGM<-calcola.moranGrayMeanz_xFixed(wMatrix)
        if("moranGrayMean" %in% whatDoYouWantBack) final.result$moranGrayMean[[patID]]<-mGM

        #ora calcola I
        stats.I<- calcola.statsTz_xFixed(mGM)
        ...
        extractedI <- extract.Iz_xFixed(wMatrix,mGM,stats.I)
        if("extractedI" %in% whatDoYouWantBack) final.result$extractedI[[patID]]<-extractedI

        #
        features <- calcola.featuresz_xFixed(extractedI)
    ...
    dataFrameI <- crea.dataframeIz_xFixed(final.result$features)
  ...
  crea.dataframeIz_xFixed <- function (features){
  ...
  calcola.featuresz_xFixed <- function(extractedI){
    Icoeff <- numeric()
    nPixel <- numeric()
...
extract.Iz_xFixed <- function(Wlist,moranGray,testResults){
  # nCoords <- numeric()
  # extractedI <- list()
  # Npixel <- list()
  Islice <- numeric()
...
calcola.statsTz_xFixed<-function(moranGray){
...
calcola.moranGrayMeanz_xFixed<-function(Wlist) {
  #ciclo sulle slice
  moranGray <-list()
  varIList <- list()
...
calcola.matrice.Wz_xFixed<-function(voxelCube,dStar, erosionMarginX, erosionMarginY, vcNonEroso, vcEroso, listaVoxelErosi, pixelSpacingX, pixelSpacingY) {
  Wlist<-list()
