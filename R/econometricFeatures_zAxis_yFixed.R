...
F01.pipeline.01_zAxis_yFixed<-function(inputGTV,dStar, erosionMarginX=0, erosionMarginY=0, whatDoYouWantBack=c("wMatrix","moranGrayMean","statsI","extractedI","dataFrameI","features")) {
  ...
  for (k in seq(1,length(inputGTV))){
    ...
    # siamo a y costante (piano x-z) --> scambio z con y e viceversa
    pixelSpacingX <- inputGTV[[k]]$geometricalInformationOfImages$pixelSpacing[1]
    pixelSpacingY <- as.numeric(inputGTV[[k]]$geometricalInformationOfImages$SliceThickness)

    # per prima cosa calcola la matrice W
    if (length(listaVoxelErosi) != 0){
      wMatrix<-calcola.matrice.Wz_yFixed(voxelCube,dStar, erosionMarginX, erosionMarginY, vcNonEroso, vcEroso, listaVoxelErosi, pixelSpacingX, pixelSpacingY)
      if("wMatrix" %in% whatDoYouWantBack) final.result$wMatrix[[patID]]<-wMatrix

      # ora calcola il moranGrayMean
      mGM<-calcola.moranGrayMeanz_yFixed(wMatrix)
      if("moranGrayMean" %in% whatDoYouWantBack) final.result$moranGrayMean[[patID]]<-mGM

      #ora calcola I
      stats.I<- calcola.statsTz_yFixed(mGM)
      ...
      extractedI <- extract.Iz_yFixed(wMatrix,mGM,stats.I)
      if("extractedI" %in% whatDoYouWantBack) final.result$extractedI[[patID]]<-extractedI

      #
      features <- calcola.featuresz_yFixed(extractedI)
  ...
  dataFrameI <- crea.dataframeIz_yFixed(final.result$features)
...
crea.dataframeIz_yFixed <- function (features){
...
calcola.featuresz_yFixed <- function(extractedI){
  Icoeff <- numeric()
  nPixel <- numeric()
...
extract.Iz_yFixed <- function(Wlist,moranGray,testResults){
  # nCoords <- numeric()
  # extractedI <- list()
  # Npixel <- list()
  Islice <- numeric()
...
calcola.statsTz_yFixed<-function(moranGray){
...
calcola.moranGrayMeanz_yFixed<-function(Wlist) {
  #ciclo sulle slice
  moranGray <-list()
  varIList <- list()
...
calcola.matrice.Wz_yFixed<-function(voxelCube,dStar, erosionMarginX, erosionMarginY, vcNonEroso, vcEroso, listaVoxelErosi, pixelSpacingX, pixelSpacingY) {
  Wlist<-list()
  ...
  for(y in seq(1,max(listaVoxelErosi[,2]))){
    cat(c("."))
    slice <- listaVoxelErosi[which(listaVoxelErosi[,2]==y),]
    WstarList <- list()
    coordsList <- list()
    vcErosoList <- list()
    for(x in seq(1,max(listaVoxelErosi[,1]))){
      if (is.null(dim(slice))) {slicez <- slice[which(slice[1]==x)]}
      else slicez <- slice[which(slice[,1]==x),]
      ...
      WstarList[[x]] <- Wstar
      coordsList[[x]] <- slicez
      vcErosoList[[x]] <- vcEroso[x,y,]
    }

    Wlist[[y]] <- list("wStarList"= WstarList,"coordsList"= coordsList, "vcErosoList"= vcErosoList)
