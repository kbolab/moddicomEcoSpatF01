...
F01.pipeline.01_xAxis_yFixed<-function(inputGTV,dStar, erosionMarginX=0, erosionMarginY=0, whatDoYouWantBack=c("wMatrix","moranGrayMean","statsI","extractedI","dataFrameI","features")) {
  ...
  for (k in seq(1,length(inputGTV))){
    ...
    if (length(listaVoxelErosi) != 0){
      wMatrix<-calcola.matrice.Wx_yFixed(voxelCube,dStar, erosionMarginX, erosionMarginY, vcNonEroso, vcEroso, listaVoxelErosi, pixelSpacingX, pixelSpacingY)
      if("wMatrix" %in% whatDoYouWantBack) final.result$wMatrix[[patID]]<-wMatrix

      # ora calcola il moranGrayMean
      mGM<-calcola.moranGrayMeanx_yFixed(wMatrix)
      if("moranGrayMean" %in% whatDoYouWantBack) final.result$moranGrayMean[[patID]]<-mGM

      #ora calcola I
      stats.I<- calcola.statsTx_yFixed(mGM)
      ...
      extractedI <- extract.Ix_yFixed(wMatrix,mGM,stats.I)
      if("extractedI" %in% whatDoYouWantBack) final.result$extractedI[[patID]]<-extractedI

      #
      features <- calcola.featuresx_yFixed(extractedI)
  ...
  dataFrameI <- crea.dataframeIx_yFixed(final.result$features)
...
crea.dataframeIx_yFixed <- function (features){
...
calcola.featuresx_yFixed <- function(extractedI){
  Icoeff <- numeric()
  nPixel <- numeric()
...
extract.Ix_yFixed <- function(Wlist,moranGray,testResults){
  # nCoords <- numeric()
  # extractedI <- list()
  # Npixel <- list()
  Islice <- numeric()
...
calcola.statsTx_yFixed<-function(moranGray){
...
calcola.moranGrayMeanx_yFixed<-function(Wlist) {
  #ciclo sulle slice
  moranGray <-list()
  varIList <- list()
...
calcola.matrice.Wx_yFixed<-function(voxelCube,dStar, erosionMarginX, erosionMarginY, vcNonEroso, vcEroso, listaVoxelErosi, pixelSpacingX, pixelSpacingY) {
  Wlist<-list()
  ...
  for(y in seq(1,max(listaVoxelErosi[,2]))){
    ...
    for(z in seq(1,max(listaVoxelErosi[,3]))){
      if (is.null(dim(slice))) {slicex <- slice[which(slice[3]==z)]}
      else slicex <- slice[which(slice[,3]==z),]
