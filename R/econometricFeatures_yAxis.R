...
F01.pipeline.01_yAxis<-function(inputGTV,dStar, erosionMarginX, erosionMarginY, pixelSpacingX=1, pixelSpacingY=1, whatDoYouWantBack=c("statsI")) {
  ...
  for (k in seq(1,length(inputGTV))){
    ...
    if (length(listaVoxelErosi) != 0){
      wMatrix<-calcola.matrice.Wy(voxelCube,dStar, erosionMarginX, erosionMarginY, vcNonEroso, vcEroso, listaVoxelErosi, pixelSpacingX, pixelSpacingY)
      if("wMatrix" %in% whatDoYouWantBack) final.result$wMatrix[[patID]]<-wMatrix

      # ora calcola il moranGrayMean
      mGM<-calcola.moranGrayMeany(wMatrix)
      if("moranGrayMean" %in% whatDoYouWantBack) final.result$moranGrayMean[[patID]]<-mGM

      #ora calcola I
      stats.I<- calcola.statsTy(mGM)
      ...
      extractedI <- extract.Iy(wMatrix,mGM,stats.I)
...
extract.Iy <- function(Wlist,moranGray,testResults){
  # nCoords <- numeric()
  # extractedI <- list()
  # Npixel <- list()
  Islice <- numeric()
...
calcola.statsTy<-function(moranGray){
...
calcola.moranGrayMeany<-function(Wlist) {
  #ciclo sulle slice
  moranGray <-list()
  varIList <- list()
...
calcola.matrice.Wy<-function(voxelCube,dStar, erosionMarginX, erosionMarginY, vcNonEroso, vcEroso, listaVoxelErosi, pixelSpacingX, pixelSpacingY) {
  Wlist<-list()
